...
class PermutationGroups:
    ...
    @staticmethod
    def conjugacy_class(n: int, classes: list[list[int]]) -> CayleyGraphDef:
        """
        Cayley graph for S_n generated by all permutations in the conjugacy classes defined by classes.
        Example: classes = [[2,2],[3]] generates all permutations with two 2-cycles and one 3-cycle.
        """
        assert n >= 1, "n must be >= 1"
        assert all(all(cl > 0 for cl in cycle) for cycle in classes), "All cycle lengths must be positive"

        generators = []
        generator_names = []

        for cycle_lengths in classes:
            cycles_sum = sum(cycle_lengths)
            lengths = sorted(cycle_lengths + [1] * (n - cycles_sum), reverse=True)
            for i, perm in enumerate(permutations_with_cycle_lenghts(n, lengths)):
                generators.append(perm)
                generator_names.append(f"({','.join(map(str, lengths))})_{i + 1}")

        name = f"conjugacy_class-{n}-{'-'.join([','.join(map(str, cl)) for cl in classes])}"
        return CayleyGraphDef.create(
            generators,
            central_state=list(range(n)),
            generator_names=generator_names,
    ...
    @staticmethod
    def conjugacy_class_rand(n: int, classes: list[tuple[int, list[int]]]) -> CayleyGraphDef:
        """
        Cayley graph for S_n generated random samples from the conjugacy classes defined by classes.
        Example: classes = [(10, [2,2]), (20, [3])] creates a Cayley graph with 10 random generators
        with two 2-cycles and 20 random generators with one 3-cycle.
        """
        assert n >= 1, "n must be >= 1"
        assert all(all(cl > 0 for cl in cycle) for _, cycle in classes), "All cycle lengths must be positive"

        generators = []
        generator_names = []

        for n_samples, cycle_lengths in classes:
            cycles_sum = sum(cycle_lengths)
            lengths = sorted(cycle_lengths + [1] * (n - cycles_sum), reverse=True)
        ...
        name = f"conjugacy_class_rand-{n}-{'-'.join([str(cl[0]) + '-' + ','.join(map(str, cl[1])) for cl in classes])}"
        return CayleyGraphDef.create(
            generators,
            central_state=list(range(n)),
            generator_names=generator_names,
...
def prepare_graph(name: str, n: int = 0, **kwargs) -> CayleyGraphDef:
    ...
    if name == "cube_2/2/2_6gensQTM":
        ...
        return PermutationGroups.conjugacy_class(n, kwargs["classes"])
    elif name == "conjugacy_class_rand":
        return PermutationGroups.conjugacy_class_rand(n, kwargs["classes"])
